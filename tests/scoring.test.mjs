...
const CCAP = S.CUT_ACC_PERFECT;
const CUT_DIFFS = ['easy', 'normal', 'hard', 'harder', 'insane'];
assert(cut(1, 'easy', 10) === Math.round(CCAP * 1.05), 'perfect easy fast = accuracy weight + 5% time bonus');
  assert(cut(1, 'easy', 999) === CCAP, 'perfect easy slow = exactly the accuracy weight (no time bonus)');
  assert(cut(1, 'easy', 1000) === CCAP, 'perfect easy capped at the accuracy weight floor');
  between(cut(1, 'easy', 10), 0, 5000, 'perfect easy bounded 0..5000');
  assert(cut(1, 'insane', 10) === Math.round(CCAP * 1.30 * 1.05), 'perfect insane fast = CCAP * 1.30 * 1.05');
  assert(cut(1, 'insane', 999) === Math.round(CCAP * 1.30), 'perfect insane slow = CCAP * 1.30');
...
const CAP = S.CIRCLE_ACC_PERFECT;
assert(circ(1, 1, 7) === CAP, 'perfect full cover at ref time = accuracy weight');
  assert(circ(1, 1, 0.1) === Math.round(CAP * 1.15), 'perfect full cover fastest = CAP * 1.15');
  assert(circ(1, 1, 999) === Math.round(CAP * 0.85), 'perfect full cover slowest = CAP * 0.85');
  assert(circ(1, 0.5, 7) === Math.round(CAP * 0.5), 'half coverage halves the score');
  assert(circ(1, 0, 7) === 0, 'no coverage = 0');
  assert(circ(1, -1, 7) === 0, 'negative coverage clamps to 0');
  assert(circ(1, 2, 7) === CAP, 'coverage above 1 clamps to perfect');
