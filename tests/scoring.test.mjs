...
assert(cut(1, 'easy', 10) === 74, 'perfect easy fast = accuracy weight + 5% time bonus');
  assert(cut(1, 'easy', 999) === 70, 'perfect easy slow = exactly the accuracy weight (no time bonus)');
  assert(cut(1, 'easy', 1000) === 70, 'perfect easy capped at the accuracy weight floor');
  between(cut(1, 'easy', 10), 0, 5000, 'perfect easy bounded 0..5000');
  assert(cut(1, 'insane', 10) === 96, 'perfect insane fast = 70 * 1.30 * 1.05 (no 5000 cap needed)');
  assert(cut(1, 'insane', 999) === 91, 'perfect insane slow = 70 * 1.30');
...
assert(circ(1, 1, 7) === 80, 'perfect full cover at ref time = accuracy weight (80)');
  assert(circ(1, 1, 0.1) === 92, 'perfect full cover fastest = 80 * 1.15');
  assert(circ(1, 1, 999) === 68, 'perfect full cover slowest = 80 * 0.85');
  assert(circ(1, 0.5, 7) === 40, 'half coverage halves the score');
  assert(circ(1, 0, 7) === 0, 'no coverage = 0');
  assert(circ(1, -1, 7) === 0, 'negative coverage clamps to 0');
  assert(circ(1, 2, 7) === 80, 'coverage above 1 clamps to perfect');
